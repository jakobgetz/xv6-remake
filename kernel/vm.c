...
// Swith h/w page table register to the kernel's page table,
// and enable paging.
void kvminithart()
{
...
pte_t *walk(pagetable_t pagetable, u64 va, int alloc)
{
    ...
    for (int level = 2; level > 0; level--)
    {
        ...
        if (*pte & PTE_V)
        {
            pagetable = (pagetable_t)PTE2PA(*pte); // get a pointer to the next page table entry
        }
        else
        {
    ...
    // pagetable now is a pointer to the level 0 index page
    return &pagetable[PX(0, va)];
}

// Look up a virtual address, return the physical data page address,
// or 0 if not mapped.
...
// process size. Returns new process size.
...
// First frees children and then frees top index page
...
// Copy len bytes from src to virtual address dstva in agiven page table.
// Return 0 on success, -1 on error.
int copyout(pagetable_t pagetable, u64 dstva, char *src, u64 len)
{
    u64 n, va0, pa0;
    ...
    while (len > 0)
    {
        ...
        u64 page_offset = dstva - va0;
        n = PGSIZE - page_offset;
        if (n > len)
        {
            n = len;
        }
        memmove((void *)(pa0 + page_offset), src, n);
...
int copyin(pagetable_t pagetable, char *dst, u64 srcva, u64 len)
{
    ...
    while (len > 0)
    {
        ...
        u64 page_offset = srcva - va0;
        n = PGSIZE - page_offset;
        if (n > len)
        {
            n = len;
        }
        memmove(dst, (void *)(pa0 + page_offset), n);
