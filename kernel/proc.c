...
// must be acquired before acquiring p->lock.
...
// If there are n o free procs, or a memory allocation fails, return 0;
...
int fork(void)
{
    ...
    if (uvmcopy(p->pagetable, new_proc->pagetable, p->sz) < 0)
    {
        freeproc(new_proc);
    ...
    *(new_proc->trapframe) = *(p->trapframe);
    ...
    // this weird lock ordering is to avoid deadlock
    // because someone could hold wait_lock and try to
    // acquire a lock on new_proc
    // in that case it would never release wait lock.
    release(&new_proc->lock);
...
// return the exit code of the process with pid.
// emmidiately returns -1 if process did not exit yet.
int waitpid(u64 pid)
{
    struct proc *pp;
...
// Print a process listing to console. For debugginf.
